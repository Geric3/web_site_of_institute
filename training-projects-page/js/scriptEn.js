export const affiche = async (e) => {
  ...
  fetch("./dataEn.json")
    ...
    .then(function (data) {
      ...
      for (let da of data) {
        thm += `
          <a href="./projects-fields-page/index-en-project-fields-page.html?id=${da.id}" data-name="${da.category}" data-id="${da.id}">
  ...
  const displayCategory = (category) => {
    ...
    for (let ca of category) {
      categories += `
      <a href="./projects-fields-page/index-en-project-fields-page.html?id=${ca.id}" data-name="${ca.category}" data-id="${ca.id}">
                <div class="cardHebergementMarseille">
                  <img
                    class="cardImageHebergementMarseille"
                    src="${ca.cover}"
    ...
    for (let i = 0; i < elt.length; i++) {
      ...
      elt[i].addEventListener("click", () => {
        ...
        if (objet.length != 0) {
          ...
          document.getElementById("herbe").innerHTML = "<h2>This specialty is currently under development.</h2>"
        }
